...
public extension StateStore {
    ...
    */
    func map<T>(state transformation: @Sendable @escaping (State) -> T) -> StateStore<T, Action> {
        StateStore<T, Action>(
            storeObject: storeObject.createChildStore(
                initialState: Void(),
    ...
    */
    func flatMap<T>(state transformation: @Sendable @escaping (State) -> T?) -> StateStore<T?, Action> {
        StateStore<T?, Action>(
            storeObject: storeObject.createChildStore(
                initialState: Void(),
                stateMapping: { state, _ in transformation(state) },
...
public extension StateStore {
     ...
     */
    func map<T>(state keyPath: KeyPath<State, T>) -> StateStore<T, Action> {
    ...
    */
    func flatMap<T>(state keyPath: KeyPath<State, T?>) -> StateStore<T?, Action> {
