...
Example usage:
 ...
 // Use DependencyEntry to inject the store instance

 extension Stores {
    @DependencyEntry var rootState = StateStore<AppRootState, Action>(AppRootState()) { state, action in
...
extension InjectableKeyPath {
    ...
    public func with<Local, Action>(
        _ initialState: Local,
        reducer: @escaping Reducer<Local, Action>
    ) -> StateStore<(Root, Local), Action> where Value == StateStore<Root, Action> {
