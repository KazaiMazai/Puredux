...
final class CancellableSideEffectsCollectionTests: XCTestCase {
    let timeout: TimeInterval = 3.0
    let observer = CancellableObserver()
...
final class SideEffectsCollectionTests: XCTestCase {
    ...
    func test_WhenStateIsRun_EffectExecuted() {
        let store = StateStore<[Effect.State], Bool>([Effect.State(), Effect.State()]) { state, action in
                state = state.map {
                    var effect = $0
                    action ? effect.run() : effect.cancel()
                    return effect
    ...
    func test_WhenStateIsRunMultipleTimes_EachEffectExecutedOnce() {
        let store = StateStore<[Effect.State], Bool>(
            [Effect.State(), Effect.State()]) { state, action in
